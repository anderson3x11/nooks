...
Le plus rapide, si le but est juste de voir tourner le projet : tout part dans un conteneur, comme en ligne. Seul Docker est nécessaire.
...
Le site répond alors sur <http://localhost:8080>. L'API sert aussi le site Angular, construit dans la même image et déposé dans son `wwwroot` : une seule origine, donc ni CORS ni second port à gérer.

Pour travailler sur le code, en revanche, mieux vaut les deux serveurs de développement et leur rechargement à chaud. Prérequis : .NET SDK 10, Node 20.19+ (ou 22.12+, ou 24+), Docker.
...
Dans les deux cas, la base est migrée au démarrage puis remplie si elle est vide : 66 lieux répartis sur treize villes, huit comptes de démonstration et plus de 160 avis. Les commentaires du jeu de départ sont volontairement génériques.

Le premier remplissage va chercher les photos sur Wikipédia et prend plusieurs minutes. En production il tourne en arrière-plan, une fois l'application en écoute : la carte se garnit donc progressivement au tout premier démarrage.
...
La proportion de vraies photos dépend de ce que Wikipédia renvoie au moment du remplissage, les lieux sans article utilisable retombant sur l'illustration générée. `Seed:FetchPhotos` à `false` coupe l'accès réseau (c'est le réglage des tests).
...
- **Photos rangées dans la base**, ce qui simplifie l'hébergement mais ne tiendra pas à grande échelle. Au-delà de quelques milliers de lieux, il faudra un stockage objet.
- **Fonds de carte CARTO et OpenStreetMap**, sur des offres gratuites qui ne couvrent pas un vrai trafic. Un fournisseur de tuiles sous contrat serait nécessaire en production.
